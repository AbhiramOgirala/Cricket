...
export default function ResultScreen() {
  ...
  // Save match to history on mount
  useEffect(() => {
  ...
  const getBattingTeam = (inn) =>
    inn?.battingTeamId === team1.id ? team1 : team2;
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
            ...
            {topScorer1 && (
              <View style={styles.highlightCard}>
                ...
                <Text style={styles.hlValue}>
                  {topScorer1.runs} <Text style={styles.hlSub}>({topScorer1.balls}b)</Text>
                </Text>
              </View>
            )}
            {topScorer2 && (
              <View style={styles.highlightCard}>
                ...
                <Text style={styles.hlValue}>
                  {topScorer2.runs} <Text style={styles.hlSub}>({topScorer2.balls}b)</Text>
                </Text>
              </View>
            )}
            {topWickets1 && (
              <View style={[styles.highlightCard, { borderColor: COLORS.danger }]}>
                ...
                <Text style={[styles.hlValue, { color: COLORS.danger }]}>
                  {topWickets1.wickets}W <Text style={styles.hlSub}>({topWickets1.runs}R)</Text>
                </Text>
              </View>
            )}
            {topWickets2 && (
              <View style={[styles.highlightCard, { borderColor: COLORS.danger }]}>
                ...
                <Text style={[styles.hlValue, { color: COLORS.danger }]}>
                  {topWickets2.wickets}W <Text style={styles.hlSub}>({topWickets2.runs}R)</Text>
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.detectionCard}>
            ...
            <View style={styles.detectionRow}>
              <StatBox
                label="Wides"
                value={(inn1?.extras?.wides || 0) + (inn2?.extras?.wides || 0)}
                color={COLORS.warning}
              />
              <StatBox
                label="No Balls"
                value={(inn1?.extras?.noBalls || 0) + (inn2?.extras?.noBalls || 0)}
                color={COLORS.danger}
              />
              <StatBox
                label="Total Extras"
