...
function TeamSection({ teamNum, teamName, setTeamName, players, setPlayers, color }) {
  const addPlayer = () => {
    if (players.length >= 11) { Alert.alert('Max 11 players per team'); return; }
  ...
  return (
    <View style={styles.teamSection}>
      ...
      {players.map((player, index) => (
        <PlayerInput key={player.id} player={player} index={index} onChange={updatePlayer} onRemove={removePlayer} />
      ))}
...
export default function SetupScreen() {
  ...
  const handleContinue = () => {
    ...
    if (validT1.length < 2 || validT2.length < 2) { Alert.alert('Error', 'Each team needs at least 2 players.'); return; }
    if (!team1Name.trim() || !team2Name.trim()) { Alert.alert('Error', 'Please enter names for both teams.'); return; }
    ...
    <SafeAreaView style={styles.safe}>
      <LinearGradient colors={[COLORS.bg_deep, COLORS.bg_dark]} style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={styles.oversRow}>
                {OVER_OPTIONS.map((o) => (
                  ...
                  >
                    <Text style={[styles.overChipText, selectedOvers === o && styles.overChipTextActive]}>{o}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
          ...
          <TeamSection teamNum={1} teamName={team1Name} setTeamName={setTeam1Name} players={team1Players} setPlayers={setTeam1Players} color={COLORS.primary} />
          <TeamSection teamNum={2} teamName={team2Name} setTeamName={setTeam2Name} players={team2Players} setPlayers={setTeam2Players} color={COLORS.secondary} />

          <TouchableOpacity onPress={handleContinue} activeOpacity={0.85} style={styles.continueWrap}>
            <LinearGradient colors={[COLORS.primary, COLORS.primary_dim]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.continueBtn}>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 11, fontWeight: '700', color: COLORS.text_muted, letterSpacing: 1.5, marginBottom: 10 },
  matchNameInput: {
    backgroundColor: COLORS.bg_card, borderRadius: 12, borderWidth: 1,
    borderColor: COLORS.border, paddingHorizontal: 16, paddingVertical: 14,
    color: COLORS.text_primary, fontSize: 16,
  },
  oversRow: { flexDirection: 'row', gap: 8, paddingVertical: 4 },
  overChip: {
    width: 52, height: 52, borderRadius: 12, backgroundColor: COLORS.bg_card,
    borderWidth: 1, borderColor: COLORS.border, alignItems: 'center', justifyContent: 'center',
  },
  overChipActive: { backgroundColor: COLORS.primary_glow, borderColor: COLORS.primary },
  ...
  teamHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, borderLeftWidth: 3, paddingLeft: 10, marginBottom: 12 },
  teamHeaderText: { fontSize: 13, fontWeight: '800', letterSpacing: 2 },
  teamNameInput: {
    backgroundColor: COLORS.bg_elevated, borderRadius: 10, borderWidth: 1,
    borderColor: COLORS.border, paddingHorizontal: 14, paddingVertical: 12,
    color: COLORS.text_primary, fontSize: 16, fontWeight: '600', marginBottom: 14,
  },
  playersLabel: { fontSize: 11, color: COLORS.text_muted, fontWeight: '600', letterSpacing: 1, marginBottom: 8 },
  playerRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  playerIndexCircle: { width: 28, height: 28, borderRadius: 14, backgroundColor: COLORS.bg_elevated, alignItems: 'center', justifyContent: 'center' },
  playerIndex: { fontSize: 12, fontWeight: '700', color: COLORS.text_muted },
  playerInput: {
    flex: 1, backgroundColor: COLORS.bg_elevated, borderRadius: 8, borderWidth: 1,
  ...
  addPlayerBtn: {
    ...
    borderRadius: 8, borderStyle: 'dashed', marginTop: 4,
  },
  addPlayerText: { fontSize: 13, fontWeight: '600' },
  continueWrap: { marginTop: 16 },
  continueBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, borderRadius: 14, paddingVertical: 16 },
  continueBtnText: { fontSize: 16, fontWeight: '800', color: COLORS.text_inverse, letterSpacing: 0.5 },
});
